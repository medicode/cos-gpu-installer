...
configure_nvidia_installation_dirs() {
  ...
  trap "{ umount /lib/modules/\"$(uname -r)\"/video; umount -fl /usr/lib/x86_64-linux-gnu ; umount /usr/bin; echo \"NVIDIA ERROR LOGS:\" && cat $NVIDIA_INSTALL_DIR_CONTAINER/nvidia-installer.log;}" EXIT
  popd
}
